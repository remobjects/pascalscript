...
uses
  ...
  uPSComponent, uPSCompiler, uPSRuntime, FMX.Memo.Types, FMX.ScrollBox,
  FMX.Memo, FMX.StdCtrls, FMX.Controls.Presentation  {Pascal Script}
  ;
...
type
  ...
  public
    ...
    procedure PSScriptCompile(Sender : TPSScript); virtual;
    procedure PSScriptExecute(Sender: TPSScript); virtual;
    procedure PSScriptAfterExecute(Sender: TPSScript);
    procedure IFPS3ClassesPlugin1CompImport(Sender : TObject;
       x : TIFPSPascalcompiler); virtual;
...
'+ CallRes(IsOK));'+sLineBreak+
''+sLineBreak+
'AddToLog(''Try to exec Delphi callback method from script...'');'+sLineBreak+
...
''+sLineBreak+
''+sLineBreak+
'AddToLog(''Try to exec script callback method from Delphi...'');'+sLineBreak+
''+sLineBreak+
'X := 444;'+sLineBreak+
'Y := 555;'+sLineBreak+
...
begin
  ...
  CallBackProc := tCallBackProc(CB);
    ...
    {$endif}
  {$ELSE}
  AddToLog('Rejected: Script callback from Delphi works only for Win + MacOS32. For other platform will be done later');
  {$endif}
