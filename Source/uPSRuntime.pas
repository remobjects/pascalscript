...
begin
  ...
  begin
    ...
    //fix error utf8 charset
    if FComp.Compile({$IFNDEF PS_NOWIDESTRING}{$IFDEF DELPHI6UP}System.{$IFDEF DELPHI2009UP}UTF8ToWidestring{$ELSE}UTF8Decode{$ENDIF}{$ENDIF}{$ENDIF}(dta)) then
    begin
      FCanAdd := False;
  ...
  begin
    ...
    //fix error utf8 charset
    if FComp.Compile({$IFNDEF PS_NOWIDESTRING}{$IFDEF DELPHI6UP}System.{$IFDEF DELPHI2009UP}UTF8ToWidestring{$ELSE}UTF8Decode{$ENDIF}{$ENDIF}{$ENDIF}(FScript.Text)) then
    begin
      FCanAdd := False;
